...
#TODO: support global volume
#TODO: support row volume
...
class App():
		...
		for i in range(13):
			...
			for scale in allScales:
				if (i < len(scale.scale)):
					...
					rowObj.scaleDataUnchanged.append(self.pluck(scale.get_frequency(i+1)))
				else:
			...
			if (self.mode==0):
				...
				for row in self.grid:
					...
					if (pressed and (row.fileObj)):
						hitData.append(row.fileObj.intData)
				...
				if (len(freqs)>0):
					...
					stream.write(playData.astype(numpy.float32).tostring())
		...
		for child in self.gui.grid.children():
			if (isinstance(child,GridFileButton)):
				if (self.mode == 0):
					...
					if ((self.mode==1) and (child.row > 7)):
						child.setText("")
					else:
						child.setText(str(child.row))
	...
	def updateRowVolume(self):
		...
		effectiveFactor = rowObj.volumeFactor*(self.gui.controlpanel.volumeSlider.value()/100.0)
